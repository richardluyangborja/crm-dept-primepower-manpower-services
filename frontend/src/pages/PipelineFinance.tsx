...
/** Pipeline hub child: per-client billing + full ledger (via Finance). */
export function PipelineFinancePage() {
  ...
  const perClientQ = useQuery({
    queryKey: ['invoices', clientId],
    queryFn: async () =>
      (await api.get('/invoices', { params: { client_id: clientId || undefined, per_page: 100 } })).data.data as Invoice[],
  });
  const perClientRows = perClientQ.data ?? [];
  const outstanding = perClientRows.filter((r) => r.status !== 'paid').reduce((a, r) => a + r.balance_centavos, 0);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <InfoCallout lead="Billing, one client or all of them.">
        Per-client detail below, full ledger at the bottom — via Finance. The CRM shows summaries, Finance owns the transactions.
      </InfoCallout>

      <section aria-label="Per client" className="flex flex-col gap-3">
        <SectionHead title="Per client" hint="Pick an account to see its invoices and outstanding balance." />
        ...
        {perClientQ.isLoading ? <p className="text-sm text-[var(--text-muted)]">Loading invoices…</p>
          : perClientQ.isError ? <div className="card p-6 text-sm">Couldn't load invoices. <button className="text-sky-600 underline" onClick={() => perClientQ.refetch()}>Retry</button></div>
          : (
            <DataTable<Invoice>
              rows={perClientRows}
              columns={[
      ...
      <section aria-label="Full ledger" className="flex flex-col gap-3">
        <SectionHead title="Full ledger" hint="Every invoice, AR aging, payments, and collection follow-ups." />
        ...
        {payId !== null && (
          <PayModal
            invoice={(invoicesQ.data ?? []).find((i) => i.id === payId) ?? perClientRows.find((i) => i.id === payId) ?? null}
