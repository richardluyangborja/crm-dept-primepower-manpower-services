...
6 users on one “Primepower Team” (manager + sales; admins/superadmin teamless) + 35 companies (15 core + 20 full-data), 30 leads, 11+ clients (+contacts), 30 opps (open stages + won with contracts/JOs/invoices), 30+ activities, 3 templates + 8+ surveys, 20 followups (10 per-company open + 10 curated lifecycle: open/overdue/snoozed/done/escalated-to-manager). Dates deterministic across 2025-11 → 2026-09 (`created_at/updated_at` set on seed so aging, trends, and renewals read realistically). Company FKs backfilled for legacy rows; zero nulls. All linked so every section/calendar has content and no screen is empty on first run.
